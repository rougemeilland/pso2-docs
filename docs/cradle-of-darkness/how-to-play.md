...
そのような状況では最高効率を求めるのは当然無理がありますが、筆者はゆりかごTの主催経験者として **せめて参加者の皆さんにはここまでは知っておいていただきたい** という知識がいくつかあります。

筆者は、本ドキュメント執筆時点で旧PSO2プレイ歴が (実質) 約13か月、ゆりかごT主催歴が約6か月の若輩者ではありますが、そのような知識を本ドキュメントにまとめてみました。どなたかのお役に立てば幸いです。
...
(緊急でもそうですが) ゆりかごTには**PSEバーストが非常に発生しやすい**という特徴があり、PSEバースト中は雑魚エネミーが大量にPOPしますので、これらを倒せば非常に多くのレアドロップや経験値を獲得できます。

つまり、**最も優先させるべきなのは如何にしてPSEバーストを発生させるか**ということになります。
...
エネミーを倒せば稀にPSEのレベルは上がっていきます。(逆にPSEが消失することもありますが)

更に、PSEのレベルが1つでも8レベルになればPSEバーストが発生します。

この辺りの詳細な仕様については、[5. 参考資料](#5-参考資料)に挙げられている資料を参照してください。

<br/>

つまり、ゆりかごTでしなければならないことは以下の内容に尽きます。

- 時間当たりのエネミー殲滅数をできるだけ増やす
- PSEバーストが発生したら全力で多くのエネミーを殲滅する
...
実際、PSEバーストの時間は (ワンモアやクロスバーストや時間延長がなければ) 1分間しかないのに集合に30秒もかかってしまった、という事例が山ほどあります。
...
基本的なセオリーとしては、フォースのプレイヤーがレ・ザンディアでエネミーを集めつつ範囲ダメージを与え、他のクラスのプレイヤーがそこに向かって範囲攻撃などを駆使してエネミーを倒しまくる、というのが一般的です。他のプレイヤーへのバフを撒いてくれるのもありがたいですね。
...
> [!TIP]
> ただし、適切な範囲攻撃手段がない場合は、範囲攻撃が届いてないエネミーを倒すのも手だと思います。
> もちろん、集合場所から離れないのを前提にですが。
