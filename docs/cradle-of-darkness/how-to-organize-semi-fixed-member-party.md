...
多分ですが、半固定トリガーの編成手順がかなり面倒なのもその大きな要因のひとつだと思います。

本ドキュメントでは、半固定トリガーを編成する方法の例について解説します。

自分で書いておいて何ですが、**本ドキュメントで説明する例でもちょっと面倒です** ので、実際に実施する価値があるかどうかは各読者のご判断にお任せします。

あと、本ドキュメントは **長い** です。いきなり結論が読みたい方は [4.2.4 というよりそもそも](#424-%E3%81%A8%E3%81%84%E3%81%86%E3%82%88%E3%82%8A%E3%81%9D%E3%82%82%E3%81%9D%E3%82%82) から読んでも大丈夫だと思います。多分。
...
部外者がパーティーの一覧から主催PTを選んで、**「別のパーティーとして参加」** を選択すると、主催PTが参加しているトリガー (つまりゆりかごT) に参加できてしまいます。
...
この設定をすることにより、主催PTと同じトリガーに (たとえ別のPTであっても) 参加しようとすると、やはりパスワード入力画面が表示され、パスワードを知らない限りトリガーに参加できなくなります。
...
主催PTで設定した **「他パーティーとのマルチプレイも制限」** の指定は、あくまでも「**主催PTと同じトリガーへの参加** にはパスワードが必要となる」指定であって、以下のように、 部外者PTが**主催PT以外の身内PTと同じトリガーに参加** しようとする分にはパスワード入力は要らずにトリガーに参加できてしまいます。
...
固定メンバーのPTのうち、身内PTの1つを **「殿 (しんがり) PT」** と呼称し、一般参加者へは **殿PTへの「別のパーティーとして参加」だけ** をできるようにするものとします。
...
事前に主催から一般参加者に「〇〇がリーダーのPTから入ってください」などの説明をしたとしても、パーティー名一覧のぱっと見だけではどれだかよくわからないので、一般参加者の椅子取り競争の負担になるだろうことは予想できます。
そもそも説明をちゃんと聞いてくれない一般参加者もおられるでしょう。
~~それに、トリガーを貼る度にがいちいち説明すること自体も面倒ですし。~~
...
- 主催PTおよび (殿PTを除く) 身内PTでは、パーティー名を ``※主催関係者専用※`` などとしておく。
...
まず、身内PTが全くない (つまり身内メンバーが主催PTのみである) 場合は、そもそも半固定トリガーにする必要がありません。普通に野良トリガーを主催すれば充分です。
...
> [!WARNING]
> 半固定トリガーを開催する場合は、一般参加者にその旨を事前に連絡しておくべきです。
> 
...
本ドキュメントの作成にあたり、鍵かけに関する仕様の調査やさまざまなスクリーンショットの撮影に協力していただいた H さんと Y さん、S さんに感謝を申し上げます。
この方たちのご協力がなければ本ドキュメントは完成しませんでした。
