...
身内PTが全くない場合 (つまり身内メンバーが主催PTのみ) は、そもそも半固定トリガーにする必要がありません。普通に野良トリガーを主催すればOKです。

身内PTが1つだけの場合 (つまり 身内PT イコール 殿PT ) は、「殿PTを除く身内PTの受注を待って殿PTも受注する」という作業をまるっと省略できるので、以下のように流れが単純になります。
